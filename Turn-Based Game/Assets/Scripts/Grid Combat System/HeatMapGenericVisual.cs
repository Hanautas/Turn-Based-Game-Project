...
public class HeatMapGenericVisual : MonoBehaviour
{
    ...
    public void SetGrid(GridSystem<HeatMapGridObject> grid)
    {
        this.grid = grid;
        UpdateHeatMapVisual();
    ...
    private void Grid_OnGridValueChanged(object sender, GridSystem<HeatMapGridObject>.OnGridValueChangedEventArgs e)
    {
        updateMesh = true;
    }
    ...
    private void UpdateHeatMapVisual()
    {
        ...
        for (int x = 0; x < grid.GetWidth(); x++)
        {
            for (int y = 0; y < grid.GetHeight(); y++)
            {
                ...
                MeshUtilities.AddToMeshArrays(vertices, uv, triangles, index, grid.GetWorldPosition(x, y) + quadSize * 0.5f, 0f, quadSize, gridValueUV, gridValueUV);
            }
        }
